...
export default function CallbackPage() {
    ...
    const fetchCode = async () => {
      try {
        ...
        if  ( body.access_token ) {
          window.localStorage.setItem('username', jwt.decode(body.id_token).name);
          window.localStorage.setItem('email', jwt.decode(body.access_token)?.sub);
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center bg-black">
      <h1 className="text-white eds-type--display-5">Authenticating... Please wait a moment</h1>
    </div>
  );
}
